...
function caDatasetForInstrumentation(inst)
{
	...
	if (inst['value-dimension'] == 1)
		return (new caDatasetScalar(inst['nsources']));

	if (inst['value-dimension'] == 3) {
		ASSERT(inst['value-arity'] == mod_ca.ca_arity_numeric);
		return (new caDatasetHeatmapDecomp(inst['nsources']));
	}

	if (inst['value-arity'] == mod_ca.ca_arity_numeric)
		return (new caDatasetHeatmapScalar(inst['nsources']));

	return (new caDatasetDecomp(inst['nsources']));
}
 ...
 */
function caDatasetSimple(zero, add, nsources)
{
	caDataset.apply(this, [ nsources ]);
	this.cds_data = {};
	this.cds_zero = zero;
	this.cds_add = add;
}
 ...
 */
function caDatasetScalar(nsources)
{
	caDatasetSimple.apply(this, [ 0, caAddScalars, nsources ]);
}
 ...
 */
function caDatasetDecomp(nsources)
{
	caDatasetSimple.apply(this, [ {}, caAddDecompositions, nsources ]);
}
 ...
 */
function caDatasetHeatmapScalar(nsources)
{
	caDatasetSimple.apply(this, [ [], caAddDistributions, nsources ]);
}
 ...
 */
function caDatasetHeatmapDecomp(nsources)
{
	caDataset.apply(this, [ nsources ]);
	this.cdh_distbykey = {};
