...
exports.ca_amqp_vers_minor		= 5;

/*
 * We use only one global exchange of type 'direct'.
 */
exports.ca_amqp_exchange 		= 'amq.direct';
exports.ca_amqp_exchange_opts		= { type: 'direct' };
...
capAmqpCap.prototype.stop = function ()
{
	...
	for (cmdid in this.cap_cmds)
		this.cap_cmds[cmdid](new caError(ECA_INTR));
...
capAmqpCap.prototype.sendCmdDisableAgg = function (route, id, aggId)
{
	var msg = {};

	msg.ca_id = id;
	msg.ca_type = 'cmd';
	msg.ca_subtype = 'disable_aggregation';
	msg.ag_inst_id = aggId;
	this.send(route, msg);
};

capAmqpCap.prototype.sendCmdAckDisableAggSuc = function (route, id, instId)
{
	var msg = {};
	...
	msg.ca_id = id;
	msg.ag_inst_id = instId;
	msg.ag_status = 'disabled';
	this.send(route, msg);
};

capAmqpCap.prototype.sendCmdAckDisableAggFail = function (route, id, error,
	instId)
{
	var msg = {};
...
capAmqpCap.prototype.sendCmdDataDelete = function (route, id, data)
{
	var msg = {};
	msg.ca_type = 'cmd';
...
capAmqpCap.prototype.sendCmdAckDataDelete = function (route, id, data)
{
	var msg = {};
	msg.ca_type = 'ack';
	msg.ca_subtype = 'data_delete';
...
capAmqpCap.prototype.cmdDataDelete = function (route, timeout, requests,
    callback)
{
	var cmdid;
	...
	this.sendCmdDataDelete(route, cmdid, requests);
};
...
capAmqpCap.prototype.cmdDisableAgg = function (route, instid, timeout, callback)
{
	var cmdid;
	...
	cmdid = this.cmd(timeout, function (err, msg) {
		...
		if (msg.ag_status != 'disabled')
			return (callback(new caError(ECA_REMOTE, null,
			    'failed to disable aggregator: %j', msg)));

		return (callback(null, true));
	});

	this.sendCmdDisableAgg(route, cmdid, instid);
};
...
capAmqpCap.prototype.cmd = function (timeout, callback)
{
	...
	/*
	 * Our ID scheme is pretty simple: "auto" plus our pid plus a
	 * monotonically increasing number.  We could have different numbers
	 * based on the hostname, but we don't bother.  All that really matters
	 * is that it be unique, and particularly that it not overlap with those
	 * that might be used by another consumer of this object sending raw
	 * commands.
	 */
	id = 'auto.' + process.pid + this.cap_cmdid++;
	this.cap_cmds[id] = function (err, msg) {
		if (timeout)
			clearTimeout(tid);
