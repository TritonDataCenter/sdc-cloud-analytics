 ...
 *
 *	dbglog	if true, print debug information for each message sent and
 *		received to the specified log
 */
function capAmqpCap(args)
{
	...
	this.cap_amqp.on('error', function (err) {
		var log = cap.cap_log;

		if (retrying) {
			/*
			 * If we're already retrying, this has failed after
			 * a restart; give up and emit the error at this point.
			 */
			log.warn('re-emitting error code ' + err.code +
			    ' on queue ' + cap.cap_source +
			    '; failed on retry after ping');
			retrying = false;
			cap.emit('error', err);
			return;
		}

		if (!err.restart || err.code != mod_caamqp.RESOURCE_LOCKED) {
			cap.emit('error', err);
			return;
		}

		/*
		 * We seem to not be able to get the exclusive lock for our
		 * queue.  This may be because we came down hard (that is,
		 * without orderly disconnects) and we have stale connection
		 * state at the AMQP server.  If this is the case, sending
		 * anything to the queue will cause the connection state to
		 * reset and the exclusive lock on the queue to be released.
		 * We therefore send a ping message (not caring about the
		 * response), wait ten seconds, and call the restart function
		 * on the error object.
		 */
		retrying = true;

		log.warn('sending ping to queue ' + cap.cap_source +
		    ' after receiving RESOURCE_LOCKED');

		cap.sendCmdPing(cap.cap_source);

		setTimeout(function () {
			log.warn('reattempting bind to queue ' +
			    cap.cap_source + ' after ping');

			err.restart(function () {
				log.warn('RESOURCE_LOCKED error on queue ' +
				    cap.cap_source + ' cleared after ping');
				retrying = false;
			});
		}, 10000);
	});
...
capAmqpCap.prototype.receive = function (msg)
{
	...
	if (this.cap_dbglog)
		this.cap_dbglog.dbg('component %s received %s/%s: %j',
		    this.cap_amqp.routekey(), type, subtype, msg);
...
capAmqpCap.prototype.send = function (routekey, msg)
{
	...
	if (this.cap_dbglog)
		this.cap_dbglog.dbg('component %s sending %s/%s to %s: %j',
		    this.cap_amqp.routekey(), type, subtype, routekey, sendmsg);
...
{
	var cap = this;
	this.cap_log.dbg('binding to routekey "%s"', routekey);
	this.cap_amqp.bind(routekey, function () {
		cap.cap_log.dbg('bind to routekey "%s" completed', routekey);
		callback();
	});
};
...
function caAbortRemote(cap)
{
	return (function (msg) {
	    cap.sendCmdAckAbort(msg.ca_source, msg.ca_id, true);
	    caPanic('panic due to remote request');
	});
}

exports.caAbortRemote = caAbortRemote;

function caPingRemote(cap)
{
	return (function (msg) {
	    cap.sendCmdAckPing(msg.ca_source, msg.ca_id);
	});
}

exports.caPingRemote = caPingRemote;
