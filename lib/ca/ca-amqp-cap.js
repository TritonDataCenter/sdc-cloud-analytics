...
capAmqpCap.prototype.receive = function (msg)
{
	...
	if (!('ca_type') in msg) {
		console.log('dropped message with unspecified type');
		return;
	}
	...
	if (!(type in cap_types)) {
		console.log('dropped message with unknown type: ' + type);
		return;
	}
	...
	if (type == 'cmd' && !('ca_id' in msg)) {
		console.log('dropped cmd message with no id');
		return;
	}

	if (!('ca_time' in msg)) {
		console.log('dropped message with unspecified time');
		return;
	}

	if (!('ca_hostname' in msg)) {
		console.log('dropped message with unspecified hostname');
		return;
	}

	if (!('ca_source' in msg)) {
		console.log('dropped message with unspecified source');
		return;
	}

	if (!('ca_subtype' in msg)) {
		console.log('dropped message with unspecified subtype');
		return;
	}
	...
	if (!(subtype in cap_types[type])) {
		console.log('dropped message with unknown subtype: ' + subtype);
		return;
	}
...
function capDispatch(cap, msg)
{
	...
	if (cap.listeners(eventname).length === 0)
		console.log('dropped ignored message of type ' + eventname);
...
function capValidate(cap, msg)
{
	if (mod_ca.caIncompatible(msg)) {
		console.log('dropped message with incompatible version');
		return;
	}
