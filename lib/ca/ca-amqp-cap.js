...
capAmqpCap.prototype.sendNotifyAggOnline = function (route, port)
{
	var msg = {};
...
/*
 * key corresponds to the stat key name
 * dim corresponds to the number of dimensions that we are using
 */
capAmqpCap.prototype.sendCmdEnableAgg = function (route, id, aggId, key, dim,
    options)
{
	var msg = {};
	...
	msg.ag_key = key;
	msg.ag_dimension = dim;
	msg.ag_options = options;
	this.send(route, msg);
};
 ...
 *  - statname
 *  - decomp
 *  - predicate (eventually, though not yet)
 */
