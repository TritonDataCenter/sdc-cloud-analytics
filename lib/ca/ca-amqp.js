...
caAmqp.prototype.send = function (key, msg)
{
	if (!this.caa_exchange)
		/*
		 * Consumers can send messages before we've actually connected
		 * or between a disconnect and reconnect event.  We queue them
		 * here and send them when we finish reconnecting.
		 */
	...
	try {
		...
		throw (new caError(ECA_UNKNOWN, ex, 'amqp error'));
	}
	return (undefined);
};
