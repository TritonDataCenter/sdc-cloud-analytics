...
caAmqp.prototype.send = function (key, msg)
{
	...
	if (this.caa_exchange) {
		try {
			...
			if (this.caa_conn.writable) {
				this.caa_log.error('amqp-error on write ' +
				    'while socket is still writable: %r', ex);
				throw (ex);
			}

			this.caa_log.warn('amqp-error on write; stream is ' +
			    'no longer writable: %r', ex);
