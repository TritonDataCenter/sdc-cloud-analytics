...
caAmqp.prototype.connSetup = function (callback)
{
	...
	this.caa_exchange.on('open', function () {
		...
		queue.bind(cfgamqp.caa_exchange_name, cfgamqp.caa_queue_name);

		for (ii = 0; ii < cfgamqp.caa_bindings.length; ii++)
			queue.bind(cfgamqp.caa_exchange_name,
			    cfgamqp.caa_bindings[ii]);

		queue.subscribe(function (msg) { cfgamqp.receive(msg); });

		if (callback)
			/* XXX need fix for bind race */
...
caAmqp.prototype.connClosed = function ()
{
	...
	if (!this.caa_closing &&
	    (!this.caa_notfirst || this.caa_ncloses >= ntries)) {
...
{
	/* XXX need queue up if caa_exchange == null */
...
caAmqp.prototype.bind = function (routekey, callback)
{
	...
	queue.bind(this.caa_exchange_name, routekey);
	/*
	 * XXX need fix for bind race.  Even then, what happens if there's an
	 * error? timeout?
	 */
	setTimeout(callback, 500);

	/*
	 * Add this to our list of bindings so we know to rebind if we have to
	 * reconnect.
	 */
 ...
 */
exports.caAmqpLogError = function (error)
{
	console.log('warning: amqp: ' + error.message);
};

exports.caAmqpFatalError = function (error)
{
