...
/*
 * Like the AMQP broker, the VMAPI/CNAPI parameters are specified via
 * environment variables.
 */
function caSdcConfig()
{
	if (!process.env['CNAPI_URL'] || !process.env['VMAPI_URL'])
		return (undefined);

	return ({
	    cnapi_url: process.env['CNAPI_URL'],
	    vmapi_url: process.env['VMAPI_URL']
	});
}

exports.caSdcConfig = caSdcConfig;
...
/*
 * Formats a time duration (given in millis)
 */
function caFormatDuration(time)
{
	var days, hours, minutes, seconds, msec, str;
	...
	if (days > 0)
		str += caSprintf('%dd ', days);

	if (days > 0 || hours > 0)
		str += caSprintf('%02d:', hours);

	if (days > 0 || hours > 0 || minutes > 0)
		str += caSprintf('%02d:', minutes);

	str += caSprintf('%02d.%03ds', seconds, msec);
	return (str);
}

exports.caFormatDuration = caFormatDuration;
...
function caHttpParam(formals, actuals, param)
{
	...
	if (!(param in actuals) || actuals[param] === undefined) {
		if (decl.required)
