...
function caHttpRouter(router)
{
	return (function (server) {
		/*
		 * We have to process OPTIONS, even if we always fail it,
		 * because our response will have the appropriate Access-Control
		 * headers that will enable the browser to make a subsequent
		 * request (e.g., DELETE).  We add our handler after our
		 * caller's handlers to make sure we don't stomp on them.
		 */
		router(server);

		/* JSSTYLED */
		server.options(/.*/, function (request, response) {
			var headers = { 'Allow': ca_http_allowed_methods };
			response.send(HTTP.EBADMETHOD, '', headers);
		});
	});
}
...
caHttpServer.prototype.initRequest = function (request, response, next)
{
	...
	response.send = function (code, body, headers) {
		...
		if (code >= HTTP.EBADREQUEST && request.method != 'OPTIONS')
			server.chs_log.warn('HTTP request failed: %s %s: %s',
			    request.method, request.url, response_data);
	...
	request.on('data', function (chunk) {
		if (request.ca_body.length + chunk.length > ca_http_maxentity) {
			response.send(HTTP.ETOOLARGE, HTTP.MSG_ETOOLARGE);
			request.destroy(); /* XXX */
			return;
		}
