...
caHttpServer.prototype.initRequest = function (request, response, next)
{
	...
	response.send = function (code, body, headers) {
		...
		server.chs_nrequests++;

		if (!server.chs_nrequests_bycode[code])
			server.chs_nrequests_bycode[code] = 0;
		server.chs_nrequests_bycode[code]++;
	};
 ...
 */
function caHttpForward(request, response, host, port, log)
{
