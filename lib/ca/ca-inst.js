 ...
 */
caInstrumentationFactory.prototype.create = function (custid, props, profile,
    callback)
{
	var request;
...
caInstrumentationFactory.prototype.stageValidate = function (request, callback)
{
	 ...
	 * we act just as though it didn't exist at all.  We check without the
	 * fields because we can't extract the fields from the predicate until
	 * after we've validated the predicate itself, and that relies on having
	 * an existing base metric.
	 */
	error = request.profile.validateMetric({
		module: props['module'],
		stat: props['stat'],
		fields: []
	});
	...
	/*
	 * Validate the predicate.  Recall that we already made sure the base
	 * metric exists by checking against the profile above.
	 */
	...
	/*
	 * Now that we've got a valid base metric and predicate, check that the
	 * user-specified fields are valid for this profile too.
	 */
	error = request.profile.validateMetric({
		module: props['module'],
		stat: props['stat'],
...
{
	var mod, stat, fields, field, type, ii, ctype;
	var ndiscrete = 0;
	var nnumeric = 0;

	ASSERT(modname in modules);
	mod = modules[modname];

	ASSERT(statname in mod['stats']);
	...
	for (ii = 0; ii < decomp.length; ii++) {
		...
		ctype = mod_ca.caTypetoArity(fields[field].type);
