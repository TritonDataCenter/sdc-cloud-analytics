...
var cfg_http_port = 23181;		/* HTTP server port */
...
function caConfigService(argv)
{
	...
	this.cfg_log = new mod_log.caLog({ out: process.stderr });

	/* log files */
	if (argv.length > 0) {
		this.cfg_dbglog = mod_log.caLogFromFile(argv[0],
		    { candrop: true }, mod_log.caLogError(this.cfg_log));
		this.cfg_log.info('Logging AMQP messages to "%s"', argv[0]);
	}

	if (argv.length > 1) {
		this.cfg_rqlog = mod_log.caLogFromFile(argv[1],
		    { candrop: true }, mod_log.caLogError(this.cfg_log));
		this.cfg_log.info('Logging HTTP requests to "%s"', argv[1]);
	}
...
caConfigService.prototype.start = function (callback)
{
	...
	setTimeout(this.tickReaper.bind(this), this.cfg_reaper_interval);
...
caConfigService.prototype.loadConfig = function (callback)
{
	...
	this.cfg_cap.cmdDataGet(mod_cap.ca_amqp_key_persist, 10000,
...
/*
 * Respond to the CA-AMQP "status" command.
 */
...
caConfigService.prototype.amqpAggOnline = function (msg)
{
	...
	for (fqid in aggr.cag_insts)
		this.cfg_factory.reenableAggregator(this.cfg_instrs[fqid]);
...
caConfigService.prototype.amqpInstrOnline = function (msg)
{
	...
	for (fqid in instr.ins_insts) {
		...
		this.cfg_factory.enableInstrumenter(instn, instr,
		    function (err) {
			if (err)
				log.error('failed to reenable "%s" on ' +
				    '"%s": %r', fqid, instr.ins_hostname, err);
		    });
	}
};
...
caConfigService.prototype.httpMetricSet = function (request)
{
	...
	if (request.params['custid'] !== undefined)
		return (this.cfg_profile_customer.metrics().intersect(mset));

	if (request.params['profile'] == 'none')
		return (mset);

	return (this.cfg_profile_operator.metrics().intersect(mset));
};

/*
 * Return the instrumentation specified by the given request URI, or null if
 * none is specified.
 */
caConfigService.prototype.httpInstn = function (request)
{
...
/*
 * Returns instrumentation properties read from request form fields or the body
 * (via JSON).
 */
...
caConfigService.prototype.httpInstnGetProps = function (request, response)
{
	...
	instn = this.httpInstn(request);
	if (!instn)
		return (response.send(HTTP.ENOTFOUND));

	return (response.send(HTTP.OK, instn.properties()));
};
...
caConfigService.prototype.httpInstnCreate = function (request, response)
{
	...
	if (custid !== undefined) {
		ninstns = custid in this.cfg_custs ?
		    Object.keys(this.cfg_custs[custid]).length : 0;
...
caConfigService.prototype.httpInstnDelete = function (request, response)
{
	...
	this.cfg_taskq.task(function (taskcb) {
		var instn = svc.httpInstn(request);
	...
	this.cfg_taskq.task(function (taskcb) {
		...
		svc.instnSetProps(instn, props, function (err) {
			if (err)
				...
				response.send(instn.properties());
...
caConfigService.prototype.instnDelete = function (instn, callback)
{
	...
	scope = custid === undefined ?
	    this.cfg_globals : this.cfg_custs[custid];
	...
	this.save(function (err) {
		...
		/*
		 * We return back to the caller as soon as we update the
		 * persistent state.  After that it's our responsibility to
		 * clean up.
		 */
		callback();
		...
		return (svc.cfg_factory.destroy(instn, function (suberr) {
			...
			svc.cfg_log.error('failure deleting instn "%s": %r',
			    fqid, suberr);
		}));
	});
};
...
caConfigService.prototype.save = function (callback)
{
	...
	this.cfg_cap.cmdDataPut(mod_cap.ca_amqp_key_persist, 10000, [ {
...
caConfigService.prototype.status = function (callback, recurse, timeout)
{
	...
	for (key in this.cfg_custs)
		ret['instn-scopes']['cust:' + key] =
		    Object.keys(this.cfg_custs[key]);
...
caInstrumentationFactory.prototype.destroy = function (inst, callback)
{
	...
	var log = this.cif_log;
		...
		return (callback(new caError(ECA_REMOTE, errors[0]['error'],
