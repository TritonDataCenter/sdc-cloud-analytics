...
{
	var metadata, data, fqid, instn, host, ii;
	...
	for (fqid in data['instns']) {
		...
		host = instn['aggregator'];

		if (!(host in this.cfg_aggrs))
			this.cfg_aggrs[host] = {
			    cag_hostname: host,
			    cag_ninsts: 0,
			    cag_insts: {}
		...
		this.cfg_aggrs[host].cag_ninsts++;
		this.cfg_aggrs[host].cag_insts[fqid] = true;

		for (ii = 0; ii < instn['instrumenters'].length; ii++) {
			host = instn['instrumenters'][ii];

			if (!(host in this.cfg_instrs))
				this.cfg_instrs[host] = {
				    ins_hostname: host,
				    ins_ninsts: 0,
				    ins_insts: {}
				};

			this.cfg_instrs[host].ins_ninsts++;
			this.cfg_instrs[host].ins_insts[fqid] = true;
		}
...
caConfigService.prototype.amqpAggOnline = function (msg)
{
	...
	for (fqid in aggr.cag_insts)
		this.cfg_factory.reenableAggregator(this.cfg_instns[fqid]);
