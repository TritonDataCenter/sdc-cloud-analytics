...
{
	var probedesc, ii, jj, elt, key, ent, ks;
	...
	for (ii = 0; ii < probedesc.length; ii++) {
		...
		if ('gather' in ks)
			mdCheckGather(elt['gather'], desc['fields']);

		if ('alwaysgather' in ks)
			mdCheckGather(elt['alwaysgather'], desc['fields']);
		...
		if ('local' in ks) {
			...
			for (jj = 0; jj < elt['local'].length; jj++) {
				...
				ASSERT(typeof (ent) == typeof ({}),
				    ...
				    'objects: %j', ent));
				ASSERT(Object.keys(ent).length == 1,
				    'each entry in local array should be an ' +
				    'object with one key');
				ASSERT(typeof (ent[Object.keys(ent)[0]]) ==
				    typeof (''), 'local entry value should ' +
				    'be a string');
			}
		}
		...
		if ('aggregate' in ks) {
			...
			for (key in elt['aggregate']) {
				...
				ASSERT(key in desc['fields'],
				    'aggregate key is not a valid field: ' +
				    key);
				ASSERT(!desc['fields'][key]['internal'],
				    'cannot aggregate an internal only field');
	...
	for (ii = 0; ii < probedesc.length; ii++) {
		...
		if ('clean' in probedesc[ii]) {
			...
			ASSERT('gather' in ks, 'cannot clean without a' +
			    'gather');
			for (key in ks['gather-keys'])
				ASSERT(key in probedesc[ii]['clean'],
				    'clean must have all variables ' +
				    'gathered, missing: ' + key);
		}
	}
...
function mdNeedProbe(probe, fields, gather)
{
	...
	if ('clean' in probe && !caIsEmpty(gather))
		return (true);
...
function mdTransformsResolvePredicate(trans, names)
{
	...
	for (ii = 0; ii < entries.length; ii++) {
		...
		require('util').debug(caSprintf('names: %j', names));
...
function mdGenerateAggregate(decomps, agg, trans)
{
	...
	for (key in decomps) {
		if (decomps[key] == mod_ca.ca_arity_numeric) {
		...
		if (decomps[key] == mod_ca.ca_arity_discrete) {
 ...
 *			have distributions.
 */
function mdGenerateDScript(desc, metric)
{
	...
	if (metric.is_zones) {
		...
		zonepred = metric.is_zones.map(function (elt) {
			return ('zonename == "' + elt + '"');
		}).join(' ||\n');
	}
	...
	for (ii = 0; ii < metric['is_decomposition'].length; ii++) {
		...
		decomps[tmp] =
		    mod_ca.caTypeToArity(desc['fields'][tmp]['type']);
		fields[tmp] = true;
	}
