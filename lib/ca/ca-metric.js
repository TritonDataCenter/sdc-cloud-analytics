 ...
 *				also specified.
 *
 *		power		If specified, "unit" and "value-base" must also
 *				be specified.  "value-power" indicates the
 *				actual unit that numbers are reported in.  If
 *				unspecified and "value-base" is specified, the
 *				value is assumed to be "1" (indicating that the
 *				reporting unit is the same as the display unit).
 ...
 *		prefixes should be applied (otherwise you'd see something like
 *		"0.000000182 seconds" instead of "182 nanoseconds"), and
 *		"value-power" is -9 to indicate that the reporting unit is 10^-9
 *		seconds (nanoseconds) rather than seconds.
 *
...
caMetricMetadata.prototype.addMetric = function (name, metric)
{
	...
	this.insert(name, this.cmm_metrics,
	    metric['module'] + '.' + metric['stat'], metric);
};
...
/*
 * Given a handle to the metadata manger, returns an instance of
 * caMetricMetadata for the global metric metadata.
 */
function caMetricMetadataRead(mdmgr)
{
	var content, metadata;

	content = mdmgr.get('metric', 'metrics');
	metadata = new caMetricMetadata();
	metadata.addFromHost(content, 'metrics.json');
	return (metadata);
}
...
exports.caMetricMetadataRead = caMetricMetadataRead;
