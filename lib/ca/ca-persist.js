	...
	function cleanupPromote(bucket) {
		...
		return (function (unused2, subcallback) {
			...
			caRename(oldpath, newpath, function (err) {
				...
				return (caRemoveTree(oldpath, subcallback));
			});
		});
	}
    ...
    callback)
{
	...
	stages.push(function (unused, subcallback) {
		...
		caRemoveTree(olddir, function (err) {
			if (err) {
				/*
	...
	caRunStages(stages, null, function (err) {
		...
		if (!written) {
			log.dbg('stash update "%s" failed; removing "%s"',
			    bucket, tmpdir);
			caRemoveTree(tmpdir, function () { callback(err); });
			return (undefined);
		}
...
function caRemoveTree(path, callback)
{
	caPanic('not yet implemented'); /* XXX */
}
