 ...
 *      instead of raw bytes, we would rev ca_bucket_version_major, since old
...
caStash.prototype.populate = function (callback)
{
	...
	mkfile = function (unused, subcallback) {
		caSaveFile(tmppath, JSON.stringify(config), function (err) {
			if (err)
				return (subcallback(new caError(err.code(), err,
 ...
 */
caStash.prototype.bucketDelete = function (bucket, callback)
{
	var stash = this;

	ASSERT(this.cas_buckets, 'caStash.bucketDelete() called before init()');

	this.bucketTask(bucket, function (taskcb) {
		stash.doBucketDelete(bucket, function (err) {
			callback(err, err ? undefined: true);
			taskcb();
		});
	});
...
caStash.prototype.bucketValidate = function (bucket, callback)
{
	...
	if (bucket.length > 0 && bucket[0] == '.') {
		callback(new caError(ECA_INVAL, null,
		    'bucket "%s" is reserved and cannot be changed', bucket));
		return (false);
	}
    ...
    callback)
{
	...
	if (!this.bucketValidate(bucket, callback))
		return (undefined);
	...
	return (caRunStages(stages, null, function (err) {
		if (!err) {
			log.info('stash update "%s" completed in %sms', bucket,
...
caStash.prototype.doBucketDelete = function (bucket, callback)
{
	var stash, stages, log, rand, realdir, tmpdir, pfx, done;
	...
	log = this.cas_log;
	rand = Math.floor(Math.random() * 10000);
	pfx = caSprintf('oldbucket-%s', bucket);
	realdir = mod_path.join(this.cas_rootdir,
	    caSprintf('bucket-%s', bucket));
	tmpdir = mod_path.join(this.cas_rootdir,
	    caSprintf('delbucket-%s-%s', bucket, rand));

	/*
	 * See doBucketFill for why the first stage has to be to remove all
	 * old buckets.  (Otherwise, if we crash later after removing this copy
	 * of the bucket, we may find one of these older turds and rename it
	 * into place on startup thinking that we crashed while updating the
	 * bucket.)
	 */
	stages.push(function (unused, subcallback) {
		log.dbg('stash delete "%s": list old dirs', bucket);
		mod_fs.readdir(stash.cas_rootdir, function (err, files) {
			if (err)
				return (subcallback(err));

			return (subcallback(null, files));
		});
	});

	stages.push(function (files, subcallback) {
		var tasks = [];

		files.forEach(function (file) {
			if (!caStartsWith(file, pfx))
				return;

			log.dbg('stash delete "%s": will remove "%s"',
			    bucket, file);
			file = mod_path.join(stash.cas_rootdir, file);
			tasks.push(function (next) {
				caRemoveTree(log, file, next);
			});
		});
		...
		caRunParallel(tasks, function (rv) {
			...
			return (subcallback(new caError(err.code(), err,
			    'failed to remove all old dirs; first error')));
		});
	});

	stages.push(function (unused, subcallback) {
		log.dbg('stash delete "%s": rename "%s"', bucket, realdir);
		caRename(realdir, tmpdir, function (err) {
			/* It's fine if the bucket didn't exist before. */
			if (err && err.code() == ECA_NOENT) {
				log.dbg('stash delete "%s": "%s" didn\'t exist',
				    bucket, realdir);
				err = undefined;
			}
	...
	stages.push(function (unused, subcallback) {
		log.dbg('stash delete "%s": remove "%s"', bucket, tmpdir);
		done = true;
		caRemoveTree(log, tmpdir, subcallback);
	});

	return (caRunStages(stages, null, function (err) {
		if (!err) {
			log.info('stash bucket "%s" deleted', bucket);
			delete (stash.cas_buckets[bucket]);
			return (callback(null));
		}

		if (done) {
			log.warn('stash bucket "%s" delete succeeded, but ' +
			    'remove failed: %r', bucket, err);
			delete (stash.cas_buckets[bucket]);
			return (callback(null));
		}

		log.info('stash bucket "%s" delete failed: %r', bucket, err);
		return (callback(err));
	}));
};
	...
	mod_fs.readFile(filename, 'utf-8', function (err, contents) {
		...
		try {
			...
			return (callback(new caError(ECA_INVAL, ex,
			    'failed to parse JSON file "%s"', filename)));
		}
