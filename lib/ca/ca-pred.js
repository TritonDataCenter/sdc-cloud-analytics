 ...
 *
 * Input:
 *  - fieldtypes: Current metric's fields and each field's type
 *  - pred: The predicate
 ...
 */
function caPredValidateRel(fieldtypes, pred, key)
{
	...
	if (!(field in fieldtypes))
		throw (new ValidationError(mod_ca.caSprintf('Metric ' +
	...
	if (arity == mod_ca.ca_arity_numeric && typeof (constant) != 'number')
	...
	if (arity != mod_ca.ca_arity_numeric && typeof (constant) != 'string')
		throw (new ValidationError(mod_ca.caSprintf('Predicate field ' +
 ...
 *
 * Input:
 *  - fieldtypes: valid fields for the current metric and their types
 ...
 */
function caPredValidateLog(fieldtypes, pred, key)
{
	var ii;
	...
	for (ii = 0; ii < pred[key].length; ii++)
		caPredValidate(fieldtypes, pred[key][ii]);
}
 ...
 *
 * Input:
 *  - fieldtypes: An object mapping valid fields of a metric to the
 *    corresponding field's type
 *  - pred: The predicate that we want to validate
 * Output: None on success, an exception is thrown on error.
 */
function caPredValidate(fieldtypes, pred)
{
	var key;

	if (!(pred instanceof Object))
		throw (new ValidationError(mod_ca.caSprintf('Predicate must ' +
		    'be an object')));
	...
	parseFuncs[key](fieldtypes, pred, key);
}
