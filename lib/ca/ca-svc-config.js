...
{
	if (this.httpReady(request, response))
		this.createInstn(request, response);
};

caConfigService.prototype.createInstn = function (request, response, tpl)
{
	var svc, custid, props, rqprops, prop, ninstns, set, log;
	...
	if (tpl) {
		props = caDeepCopy(tpl.properties());
	...
	log.info('request to instrument%s:\n%j',
	    tpl ? ' (based on instn ' + tpl.fqid() + ')' : '', props);
...
caConfigService.prototype.httpInstnClone = function (request, response)
{
	...
	oinstn = this.httpInstn(request, response);
	if (!oinstn)
		return;

	this.createInstn(request, response, oinstn);
};
...
caInstrumentationFactory.prototype.stageValidate = function (request, callback)
{
	...
	props['value-scope'] = this.cif_metadata.metricInterval(
	    props['module'], props['stat']);
