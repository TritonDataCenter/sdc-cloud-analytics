 ...
 * XXX migrate path
 * XXX test plan:
 *	o make sure extension of global metrics to new instrs still works
 */
 ...
 *
 *	    scope_id		scope (customer) id
 *
 *	    instn_id		per-customer instrumentation id
 *
 *	    properties		last known property valeus
 *
 *	    zonesbyhost		zones to instrument, by hostname
...
caConfigService.prototype.loadConfig = function (callback)
{
	...
	this.cfg_cap.cmdDataGet(mod_cap.ca_amqp_key_stash, 10000,
		...
		/*
		 * XXX
		 * if ('result' in results[1] && false)
		 * 	return (svc.migrateConfig(results[1], callback));
		 * else if (results[1]['error']['code'] != ECA_NOENT)
		 *     return (callback(new caError(ECA_REMOTE, null,
...
caConfigService.prototype.instnLoad = function (instnconf, bktid)
{
	var instn, hostname;
	...
	if (instn.cfi_scopeid === undefined) {
		this.cfg_globals[instn.cfi_fqid] = true;
		...
		this.cfg_custs[instn.cfi_scopeid]['insts'][instn.cfi_fqid] =
		    true;

		this.cfg_custs[instn.cfi_scopeid]['id'] = Math.max(
		    this.cfg_custs[instn.cfi_scopeid]['id'],
		    instnconf['instn_id'] + 1);
	}
...
{
	this.cfg_log.info('skipping config migration (NYI)'); /* XXX */
	callback();
	// for (bktkey in results[0]['result']) {
	//
	// }
 ...
 * since we don't want to persist this configuration.
 */
...
caConfigService.prototype.amqpInstrOnline = function (msg)
{
	...
	for (fqid in this.cfg_instns) {
		...
		/*
		 * By definition if this instr is assigned to some instn, this
		 * can't be the first time we've seen it.  We check this so that
		 * we don't inadvertently add both a ZonesUpdate task (see
		 * above) and a InstrsUpdate task (see below).
		 */
		mod_assert.equal(action, 'restarted');
...
caConfigService.prototype.httpInstnDelete = function (request, response)
{
	...
	if (!(instn = svc.httpInstn(request, response)))
		return;
...
caConfigService.prototype.pickAggregator = function ()
{
	...
	for (hostname in this.cfg_aggrs) {
		...
		if (aggr.cag_ninsts >= cai_agg_maxinsts)
			continue;
...
caConfigService.prototype.instnTaskSave = function instnTaskSave(instn,
    callback, props)
{
	var svc, metadata, saveobj, func, rq;
	...
	if (!instn.cfi_deleted) {
		...
		saveobj = {
			scope_id: instn.cfi_scopeid,
			instn_id: instn.cfi_instnid,
			properties: props
		};

		if (instn.cfi_zonesbyhost !== undefined)
			saveobj['zonesbyhost'] = instn.cfi_zonesbyhost;

		if (instn.cfi_aggr !== undefined)
			saveobj['aggregator'] = instn.cfi_aggr.cag_hostname;

		rq = {
		    bucket: 'ca.config.instn.' + instn.cfi_fqid,
		    metadata: metadata,
    ...
    (instn, callback)
{
	...
	this.cfg_cap.cmdEnableAgg(aggrkey, instn.cfi_fqid, instnkey,
	    instn.cfi_props, cai_timeout_aggenable, function (err) {
		if (err) {
    ...
    (instn, callback)
{
	...
	this.cfg_cap.cmdDisableAgg(aggrkey, instn.cfi_fqid,
	    cai_timeout_aggenable, function (err) {
		if (err) {
    ...
    (instn, callback)
{
	...
	for (hostname in instn.cfi_instrs)
		instn.cfi_instrs['desired'] = false;
	 ...
	 */
	if (instn.cfi_props['nsources'] != nsources) {
	...
	for (hostname in instn.cfi_instrs) {
		...
		if (!(this.cfg_metrics.supports(metric, fields, hostname))) {
			this.instnDbg(instn, true, 'instr "%s" update ' +
	...
	caRunParallel(funcs, function () {
		 ...
		 */
		svc.instnDbg(instn, true, 'instrs update done');
		callback();
	});
};
...
caConfigService.prototype.instrEnable = function (instn, hostname, callback)
{
	...
	return (this.cfg_cap.cmdEnableInst(instr.ins_routekey, instn.cfi_fqid,
	    instnkey, instn.cfi_props, zones, cai_timeout_instenable,
	    function (err) {
		if (err) {
			svc.instnDbg(instn, true, 'instr "%s" enable ' +
			    'failed: %r', hostname, err);
		} else {
			svc.instnDbg(instn, false, 'instr "%s" enable done',
			    hostname);
...
caConfigService.prototype.instrDisable = function (instn, hostname, callback)
{
	...
	return (this.cfg_cap.cmdDisableInst(instr.ins_routekey, instn.cfi_fqid,
	    cai_timeout_instdisable, function (err) {
		if (err) {
			svc.instnDbg(instn, true, 'instr "%s" disable ' +
			    'failed: %r', hostname, err);
		} else {
			svc.instnDbg(instn, false, 'instr "%s" disable done',
			    hostname);
...
function caNormalizeProperties(baseuri, scopeid, instnid, props)
{
	...
	uri = caSprintf('%s%s%s/%s', baseuri,
	    (scopeid ? cai_http_uri_cust + '/' + scopeid : ''),
	    cai_http_uri_inst, instnid);
	nprops['uri'] = uri;
