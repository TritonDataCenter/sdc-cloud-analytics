...
caConfigService.prototype.stop = function (callback)
{
	...
	svc.cfg_log.info('pending: %d', this.cfg_npending);
...
caConfigService.prototype.migrateConfig = function (legacybucket, callback)
{
	...
	rqs = this.migrateScope(undefined, data['global'], data['instns']);
	for (custid in data['customers'])
		rqs = rqs.concat(this.migrateScope(custid,
		    data['customers'][custid]['insts'], data['instns']));

	/*
	 * Now that we've assembled the list of "put" requests, send those
	 * requests to the stash, delete the original config, and goto top.
	 */
	stages = [];
...
/*
 * Returns a function for use with caRunParallel which saves new configuration
 * objects for each of the instrumentations in "scope" whose properties are
 * described by "instns".
 */
caConfigService.prototype.migrateScope = function (scopeid, scope, instns)
{
	var svc, rqs;
	...
	Object.keys(scope).forEach(function (fqid) {
		var ent = instns[fqid];
		rqs.push(svc.instnSaveRequest(ent['cust_id'], ent['inst_id'],
...
/*
 * Generates the stash "put" request to save the configuration for the
 * instrumentation with the specified properties.
 */
