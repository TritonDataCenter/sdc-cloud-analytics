 ...
 *
 *	o contacting SDC to retrieve the list of zones a user is authorized to
 ...
 *	o aggregator state
 *	o list of hosts/zones (retrieved from VMAPI/CNAPI)
...
function caConfigService(argv, loglevel)
{
	...
	this.cfg_sdccfg = mod_ca.caSdcConfig();
	...
	/* external services */
	if (this.cfg_sdccfg)
		this.cfg_sdc = new mod_sdc.caSdc(this.cfg_sdccfg,
		    this.cfg_log);
...
caConfigService.prototype.start = function (callback)
{
	...
	if (this.cfg_sdc) {
		log.info('%-12s %s', 'VMAPI url:', this.cfg_sdccfg.vmapi_url);
		log.info('%-12s %s', 'CNAPI url:', this.cfg_sdccfg.cnapi_url);
	} else {
		log.warn('VMAPI/CNAPI parameters not set.  ' +
		    'Per-customer use disabled.');
	}
...
caConfigService.prototype.loadConfig = function (callback)
{
	 ...
	 */
	this.cfg_cap.cmdDataGet(mod_cap.ca_amqp_key_stash, 60000,
...
caConfigService.prototype.instnLoad = function (instnconf)
{
	...
	/*
	 * Make sure the aggregator and instrumenters know about this
	 * instrumentation.  Then update the list of zones/hosts that we need to
	 * instrument.  This may trigger another instrumenter update, but we
	 * don't want the initial one blocked on contacting VMAPI as long as we
	 * have some appropriate cached value.
	 */
	this.instnTask(instn, this.instnTaskAggrUpdate);

	/*
	 * In some cases we didn't save zonesbyhost with per-customer
	 * instrumentations.  For those cases we set it to a safe value here,
	 * but we don't want to trigger an instrumenter update until we've set
	 * it to something reasonable.
	 */
	if (instn.cfi_scopeid === undefined ||
	    instn.cfi_zonesbyhost !== undefined)
		this.instnTask(instn, this.instnTaskInstrsUpdate);
	else
		instn.cfi_zonesbyhost = {};
...
{
	var base = this.cfg_http_uri_base;
	var url = request.url;
	...
	case base + '/docs/':
		...
		/*jsl:fallthru*/
	case base + '/docs/index.html':
	case base + '/docs/media/css/restdown.css':
	case base + '/docs/resources/heatmap.png':
		break;
	default:
		response.send(HTTP.ENOTFOUND);
		return;
	...
	mod_cahttp.caHttpFileServe(request, response, '.' + url);
};
...
{
	var instn, aggr, ipaddr, port, extraheaders;
	...
	extraheaders = {
	    'x-ca-instn-arity': instn.cfi_props['value-arity'],
	    'x-ca-instn-granularity': instn.cfi_props['granularity']
	};

	return (mod_cahttp.caHttpForward(request, response, ipaddr, port,
	    extraheaders, this.cfg_log));
};
...
caConfigService.prototype.instnValidateScope = function (scopeid)
{
	...
	/*
	 * Customer requests require that VMAPI be configured.
	 */
	if (!this.cfg_sdc)
		throw (new caError(ECA_REMOTE, null,
		    'VMAPI/CNAPI are not configured'));
...
/*
 * Updates the list of zones and hosts for this customer from VMAPI.  This
    ...
    (instn, callback)
{
	...
	/*
	 * We checked whether SDC was configured when this instrumentation was
	 * created.  Although it's possible that it was configured then but
	 * isn't now, that can't happen in production because SDC is always
	 * configured in production.
	 */
	mod_assert.ok(this.cfg_sdc, 'VMAPI/CNAPI is not configured');

	return (this.cfg_sdc.listContainers(instn.cfi_scopeid,
		...
		svc.instnTask(instn, svc.instnTaskSave);
    ...
    (instn, callback)
{
	...
	/*
	 * All per-customer instrumentations must have cfi_zonesbyhost set.
	 * Otherwise, the instrumenter won't know to scope it to a particular
	 * set of zones and the customer would be able to see everything.
	 */
	mod_assert.ok(instn.cfi_scopeid === undefined ||
	    instn.cfi_zonesbyhost !== undefined);
	...
	for (hostname in instn.cfi_instrs) {
		/*
		 * We may see a hostname reported by VMAPI that has no
