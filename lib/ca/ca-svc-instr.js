...
function caInstrService(argv, out, backends)
{
	...
	if (argv.length > 2) {
		...
		this.ins_dbglog.info('Logging AMQP message to "%s"', argv[1]);
	}
 ...
 */
caInstrService.prototype.initBackends = function ()
{
	/*
	...
	for (ii = 0; ii < this.ins_backends.length; ii++) {
		this.ins_current_backend = this.ins_backends[ii];

		try {
			plugin = require('../../cmd/cainst/modules/' +
			    this.ins_backends[ii]);
		} catch (ex) {
			this.ins_log.warn('FAILED to load module "%s": %r',
			    this.ins_backends[ii], ex);
			continue;
		}

		plugin.insinit(bemgr, this.ins_log);
		this.ins_log.info('loaded module "%s".', this.ins_backends[ii]);
	}

	if (mod_ca.caIsEmpty(this.ins_impls))
		throw (new Error('No metrics registered.  Bailing out.'));

	this.ins_current_backend = '<unknown>';
	this.ins_log.info('finished loading modules.');
};
...
caInstrService.prototype.amqpDisableInstn = function (msg)
{
	...
	svc.ins_instns[id].is_impl.deinstrument(function (err) {
		if (err) {
