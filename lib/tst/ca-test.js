...
exports.ctDoExitSuccess = function ()
{
 ...
 */
function DummyInstrumenter(conf)
{
	var dummy = this;
	...
	this.di_metrics = [ {
	    cam_name: 'test_module',
	    cam_description: 'test module description',
	    cam_stats: [ {
		cas_name: 'ops1',
		cas_description: 'test ops 1',
		cas_type: 'ops',
		cas_fields: [ {
		    caf_name: 'hostname',
		    caf_description: 'server host name',
		    caf_type: mod_ca.ca_type_string
		}, {
		    caf_name: 'latency',
		    caf_description: 'duration of operation',
		    caf_type: mod_ca.ca_type_latency
		} ]
	    } ]
	} ];
	...
	this.di_cap.on('msg-notifiy-configsvc_online',
	    function (msg) { dummy.receiveConfigSvcOnline(msg); });
	/* XXX should handle ping and status in a base class. */
}
...
{
	var dummy = this;
	this.di_cap.cap_amqp.start(function () {
		dummy.notifyConfig();
		exports.ctWaitForAmqpService(
		    mod_ca.ca_amqp_key_config, callback);
	});
};
...
{
	this.di_cap.sendNotifyInstOnline(mod_ca.ca_amqp_key_config,
	    this.di_metrics);
};
...
function DummyAggregator()
{
	...
	this.da_cap.on('msg-cmd-enable_aggregation',
	    function (msg) { dummy.receiveEnable(msg); });
	this.da_cap.on('msg-notifiy-configsvc_online',
	    function (msg) { dummy.receiveConfigSvcOnline(msg); });
	/* XXX should handle ping and status in a base class. */
}
...
{
	var dummy = this;
	this.da_cap.cap_amqp.start(function () {
		dummy.notifyConfig();
		exports.ctWaitForAmqpService(
		    mod_ca.ca_amqp_key_config, callback);
	});
...
{
	this.da_cap.sendNotifyAggOnline(mod_ca.ca_amqp_key_config, '8080', {});
};
    ...
    fatal, callback)
{
	...
	fail = function (err, rspdata) {
		log.error('HTTP request FAILED: %s %s, headers = %j, ' +
		    'body = %s; rspdata = %j; exception follows', method, uri,
		    headers, body, rspdata);
		log.exception(err);
