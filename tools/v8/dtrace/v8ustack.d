 ...
 * function is defined.
 *
 * This helper is already integrated into Node.js, but it could conceivably be
 * used for any program using V8.  To do this, you must first build an object
 * file with it using:
 *
 *	# dtrace -C -G -o v8ustack.o -s v8ustack.d
 *
 * Then link this object file with the final program that uses V8.
 * Alternatively, you can link this object file into libv8.so itself.  (ustack
 * helpers inside static libraries are not currently supported, so you cannot
 * simply link this object into libv8.a.)
 *
 * To use the helper, run the program, then use the jstack() DTrace action to
 * capture a JavaScript stacktrace.  You will probably need to tune up the
 * dtrace_helper_actions_max kernel variable to 128.
 */
